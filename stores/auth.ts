...
interface AuthState {
  ...
  signUp: (name: string, email: string, password: string, role: 'consumer' | 'seller', signupToken: string, phoneNumber?: string) => Promise<boolean>;
  logout: () => Promise<void>;
...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  signUp: async (name, email, password, role, signupToken, phoneNumber) => {
    set({ isLoading: true, error: null });
    const res = await authService.signUp({ name, email, password, role, phoneNumber, signupToken });
    if (!res.success) {
