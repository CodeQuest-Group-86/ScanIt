...
export default function HomeScreen() {
  ...
  const { authenticScans, suspiciousScans } = history.reduce(
    (acc, scan) => {
      if (scan.authenticityStatus === "authentic") {
        acc.authenticScans++;
      } else {
        acc.suspiciousScans++;
      }
          ...
          >
            <View style={styles.scanCtaText}>
              ...
              <Text style={styles.scanCtaSubtitle}>
                Point camera · prices, sellers & authenticity
              </Text>
            </View>
            ...
            <View style={styles.section}>
              ...
              {history.slice(0, 3).map((scan) => (
                <RecentScanCard
                  ...
                  onSave={() =>
                    isSaved(scan.product.id)
                      ? remove(scan.product.id)
                      : save(scan.product)
                  }
                />
              ))}
          ...
          {history.length === 0 && (
            <GlassCard intensity={50} style={styles.emptyWrap}>
              ...
              <Text style={styles.emptyBody}>
                Point your camera at any product to see what it is, where to buy
                it, the price, and whether it&apos;s genuine.
              </Text>
            </GlassCard>
          )}
        </TabScrollView>
